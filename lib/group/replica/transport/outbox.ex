defmodule Group.Replica.Transport.Outbox do
  ...
  `:erlang.send_nosuspend/3`. A sideband adapter delegates `try_send/5` to
  `try_send/5`, which performs only a local `send/2` to the matching shard
  outbox.

  Each outbox batches frames by target node outside the Group shard. Expired
  frames and batches rejected by the backend are deliberately dropped;
  ...
  A backend using this helper implements:
      ...
      def send_batch(target_node, frames, deadline, backend_state) do
        # Return promptly once `deadline` has passed. It is safe to drop.
        {:ok, backend_state}
      end

  The backend is responsible for authenticated ingress and must pass only
  complete logical frames to `Group.Replica.Transport.deliver_batch/4`.

  ## Options

    * `:outbox_batch_size` - maximum logical frames collected per flush,
      default `64`
    ...
    * `:outbox_flush_interval` - maximum batching delay in milliseconds,
      default `1`
    * `:outbox_deadline` - maximum useful residence time for an outbound frame
  ...
  frames are independently bounded by `:replicated_snapshot_chunk_target_bytes`.
  Other logical frames or a whole batch may still exceed `:outbox_batch_bytes`;
  a transport with a smaller finite frame size must segment and completely
  reassemble those batches before local delivery.
  """

  @type frame :: Group.Replica.Transport.frame()
  @type send_result :: Group.Replica.Transport.send_result()
  ...
  @doc """
  Enqueues a frame into its local shard outbox.

  This performs no backend or socket operation. `:ok` only means the message
  was sent to the current local outbox PID; the outbox may later drop it on
  expiry or backpressure. A concurrently terminating outbox can also lose an
  accepted message, which anti-entropy repairs.
  """
  def try_send(group, target_node, shard, frame, opts)
    ...
    case Process.whereis(name(group, shard)) do
      pid when is_pid(pid) ->
        ...
        send(pid, {:group_replica_outbox_send, target_node, deadline, frame})
        :ok
...
defmodule Group.Replica.Transport.Outbox.Worker do
  ...
  @impl true
  def handle_info({:group_replica_outbox_send, target_node, deadline, frame}, state)
      when is_atom(target_node) and is_integer(deadline) do
    if deadline <= Outbox.monotonic_ms() do
      {:noreply, state}
    else
      bytes = :erlang.external_size({target_node, frame})
      ...
      state = enqueue(state, target_node, deadline, frame, bytes)
  ...
  defp enqueue(state, target_node, deadline, frame, bytes) do
    entry = {target_node, deadline, frame}
  ...
  defp flush(state) do
    ...
    batches =
      state.pending
      |> Enum.reverse()
      |> Enum.reject(fn {_target_node, deadline, _frame} -> deadline <= now end)
      |> Enum.group_by(fn {target_node, _deadline, _frame} -> target_node end)

    backend_state =
      Enum.reduce(batches, state.backend_state, fn {target_node, entries}, backend_state ->
        frames = Enum.map(entries, fn {_target_node, _deadline, frame} -> frame end)

        deadline =
          entries
          |> Enum.map(fn {_target_node, deadline, _frame} -> deadline end)
          |> Enum.min()

        if deadline <= Outbox.monotonic_ms() do
          backend_state
        else
          case state.backend.send_batch(target_node, frames, deadline, backend_state) do
